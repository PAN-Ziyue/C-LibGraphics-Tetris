...
gs_button_color = {
	"Dark Gray", "White", "Light Gray", "Black", 1 , // 填充
},

gs_textbox_color = {
	"Light Gray", "White",	"White", "White",   0 , 
};
...
void uiGetTimer(int ID)
{
	gs_UIState.TimerIDInput = ID;
}

void gameGetKeyboard(int key, int event)
{
	switch (key)
	{
	case VK_SPACE:gs_GMAEState.SpaceButton = event == VK_DOWN ? 1 : 0; break;
	case VK_LEFT:gs_GMAEState.LeftButton= event == VK_DOWN ? 1 : 0; break;
	case VK_RIGHT:gs_GMAEState.RightButton= event == VK_DOWN ? 1 : 0; break;
	case VK_UP:gs_GMAEState.UpButton= event == VK_DOWN ? 1 : 0; break;
	case VK_DOWN:gs_GMAEState.DownButton= event == VK_DOWN ? 1 : 0; break;
	}
}
 ...
 *   1 - 点击了按钮
 */
int button(int id, double x, double y, double w, double h, char *label, char *tip)
{
	...
	if (gs_UIState.kbdItem == id && (gs_UIState.keyPress == VK_DOWN || gs_UIState.keyPress == VK_UP))
	{
