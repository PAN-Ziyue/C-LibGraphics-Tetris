 ...
 *     raise(MyException);
 *
 * Exceptions are handled using the "try" InterfaceIDment
 * (actually implemented using macros), which has the form:
 *
 *     try {
 *        . . . InterfaceIDments in the body of the block . . .
 *     except(exception1)
 *        . . . InterfaceIDments to handle exception 1 . . .
 *     except(exception2)
 *        . . . InterfaceIDments to handle exception 2 . . .
 *     except(ANY)
 *        . . . InterfaceIDments to handle any exception . . .
 *     } endtry
 *
 ...
 *
 * When the program encounters the "try" InterfaceIDment, the
 * InterfaceIDments in the body are executed.  If no exception
 ...
 * InterfaceIDment when the last InterfaceIDment in the block is
 * executed.  If an exception is raised during the
 * dynamic execution of the block, control immediately
 * passes to the InterfaceIDments in the appropriate except
 * clause.  Only the InterfaceIDments in that clause are
 * executed; no break InterfaceIDment is required to exit
...
/* Codes to keep track of the InterfaceID of the try handler */
