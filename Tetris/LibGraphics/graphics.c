...
/*
 * Type: graphicsInterfaceIDT
 * --------------------
 * This structure holds the variables that make up the graphics InterfaceID.
 */

typedef struct graphicsInterfaceIDT {
    double cx, cy;
    string font;
    int size;
    int style;
    bool erase;
    int color;
    struct graphicsInterfaceIDT *link;
} *graphicsInterfaceIDT;
...
/*
 * Type: regionInterfaceIDT
 * ------------------
 * The region assembly process has the character of a finite InterfaceID
 * machine with the following four InterfaceIDs:
 *
 ...
 *
 * The current InterfaceID determines whether other operations are legal
 * at that point.
 */
 ...
 * regionInterfaceID    -- Current InterfaceID of the region
 ...
 * InterfaceIDStack     -- Stack of graphicInterfaceIDT blocks
 ...
 * penColor       -- Color of pen            | InterfaceID
 */
...
static HDC gdc, osdc;
static HBITMAP osBits,BACKBITMAP;
...
static regionInterfaceIDT regionInterfaceID;
...
static graphicsInterfaceIDT InterfaceIDStack;
...
static void InitGraphicsInterfaceID(void);
...
void InitGraphics(void)
{
    if (!initialized) {
        initialized = TRUE;
        ProtectVariable(InterfaceIDStack);
...
{
    InitCheck();
    if (regionInterfaceID == RegionActive) regionInterfaceID = PenHasMoved;
    cx = x;
    cy = y;
}
...
void SaveGraphicsInterfaceID(void)
{
    graphicsInterfaceIDT sb;

    InitCheck();
    sb = New(graphicsInterfaceIDT);
    sb->cx = cx;
    sb->cy = cy;
    sb->font = textFont;
    sb->size = pointSize;
    ...
    sb->erase = eraseMode;
    sb->color = penColor;
    sb->link = InterfaceIDStack;
    InterfaceIDStack = sb;
}

void RestoreGraphicsInterfaceID(void)
{
    graphicsInterfaceIDT sb;

    InitCheck();
    if (InterfaceIDStack == NULL) {
        Error("RestoreGraphicsInterfaceID called before SaveGraphicsInterfaceID");
    }
    sb = InterfaceIDStack;
    cx = sb->cx;
    cy = sb->cy;
...
/*
 * Function: InitGraphicsInterfaceID
 * Usage: InitGraphicsInterfaceID();
 * ---------------------------
 * This function initializes the graphics InterfaceID elements to
 * their default values.  Because the erase mode and font
 * information are also maintained in the display InterfaceID,
 * InitGraphicsInterfaceID must call functions to ensure that these
...
{
	RECT r;
	HBITMAP BackGround = (HBITMAP)LoadImage(NULL, "GameBack.bmp", IMAGE_BITMAP, 720, 720, LR_LOADFROMFILE);

	SetRect(&r, 0, 0, pixelWidth, pixelHeight);
	InvalidateRect(graphicsWindow, &r, TRUE);
	SelectObject(osdc, BackGround);
}
...
static void InitColors(void)
{
    ...
    DefineColor("Dark Gray", .15, .15, .15);
    DefineColor("Gray", .6, .6, .6);
    DefineColor("Light Gray", .95, .95, .95);
...
double PtoI(int t)//pixel to inches
{
	return (double)t / GetXResolution();
}

void PIXEL(int x, int y, int r, int g, int b)//生成像素点
{
	SetPixel(osdc, x, 720-y, RGB(r, g, b));
}
...
{
	RECT r;
	HBITMAP BackGround = (HBITMAP)LoadImage(NULL, "Pause.bmp", IMAGE_BITMAP, 720, 720, LR_LOADFROMFILE);
...
void DisplayPause1(void)
{
	RECT r;
	HBITMAP BackGround = (HBITMAP)LoadImage(NULL, "CountDown1.bmp", IMAGE_BITMAP, 720, 720, LR_LOADFROMFILE);
...
void DisplayPause2(void)
{
	RECT r;
	HBITMAP BackGround = (HBITMAP)LoadImage(NULL, "CountDown2.bmp", IMAGE_BITMAP, 720, 720, LR_LOADFROMFILE);
...
void DisplayPause3(void)
{
	RECT r;
	HBITMAP BackGround = (HBITMAP)LoadImage(NULL, "CountDown3.bmp", IMAGE_BITMAP, 720, 720, LR_LOADFROMFILE);
...
void DisplayLose(void)
{
	...
	//在输掉比赛后结束gamebgm01，回到主菜单后播放bgm01
	RECT r;
	HBITMAP BackGround = (HBITMAP)LoadImage(NULL, "Lose.bmp", IMAGE_BITMAP, 720, 720, LR_LOADFROMFILE);
...
void DisplayQuitAlert(void)
{


	RECT r;
	HBITMAP BackGround = (HBITMAP)LoadImage(NULL, "quit.bmp", IMAGE_BITMAP, 720, 720, LR_LOADFROMFILE);
...
{
	RECT r;
	HBITMAP BackGround = (HBITMAP)LoadImage(NULL, "GameMenu.bmp", IMAGE_BITMAP, 720, 720, LR_LOADFROMFILE);
...
void DisplayInstuction1(void)
{
	RECT r;
	HBITMAP BackGround = (HBITMAP)LoadImage(NULL, "instuction 1.bmp", IMAGE_BITMAP, 720, 720, LR_LOADFROMFILE);
...
void DisplayInstuction2(void)
{
	RECT r;
	HBITMAP BackGround = (HBITMAP)LoadImage(NULL, "instuction 2.bmp", IMAGE_BITMAP, 720, 720, LR_LOADFROMFILE);
...
void DisplayInstuction3(void)
{
	RECT r;
	HBITMAP BackGround = (HBITMAP)LoadImage(NULL, "instuction 3.bmp", IMAGE_BITMAP, 720, 720, LR_LOADFROMFILE);
